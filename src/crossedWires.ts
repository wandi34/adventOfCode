...
export function crossedWires(wire1: string[], wire2: string[]): number {
  const cursorList_w1: Cursor[] = iterateWire(wire1)
  console.log("iterate wire 1: " + cursorList_w1.length)
  const cursorList_w2: Cursor[] = iterateWire(wire2)
  console.log("iterate wire 2: " + cursorList_w2.length)

  const crossedCursors: Cursor[] = Array.from(getSameElements(cursorList_w1, cursorList_w2))
  console.log(crossedCursors)
  const distances: number[] = crossedCursors.map(c => Math.abs(c.x) + Math.abs(c.y))
  console.log("Distances: " + distances.sort((a, b) => a - b))
  return distances.sort((a, b) => a - b)[0]
}

function getSameElements(cursorList1: Cursor[], cursorList2: Cursor[]): Set<Cursor> {
  let result: Set<Cursor> = new Set()
  const cursorSet1 = new Set(cursorList1)
  const cursorSet2 = new Set(cursorList2)
  for (var cursor of cursorList1) {
    if (cursorSet2.has(cursor) && !_.isEqual(cursor, new Cursor(0,0))) {
      result.add(cursor)
    }
  }
  return result
...
function iterateWire(wire: string[]): Cursor[] {
  let cursorList: Cursor[] = [new Cursor(0, 0)];
  wire.map(rule => {
...
function rightAdd(steps: number, cursorList: Cursor[]): Cursor[] {
  while (steps > 0) {
    const lastCursor = cursorList[cursorList.length - 1];
    cursorList.push(new Cursor(lastCursor.x +1, lastCursor.y))
    --steps
  }
  return cursorList
}

function leftAdd(steps: number, cursorList: Cursor[]): Cursor[] {
  while (steps > 0) {
    const lastCursor = cursorList[cursorList.length - 1];
    cursorList.push(new Cursor(lastCursor.x  - 1, lastCursor.y))
    --steps
...
function upAdd(steps: number, cursorList: Cursor[]): Cursor[] {
  while (steps > 0) {
    const lastCursor = cursorList[cursorList.length - 1];
    cursorList.push(new Cursor(lastCursor.x, lastCursor.y  + 1))
    --steps
  }
  return cursorList
}

function downAdd(steps: number, cursorList: Cursor[]): Cursor[] {
  while (steps > 0) {
    const lastCursor = cursorList[cursorList.length - 1];
    cursorList.push(new Cursor(lastCursor.x, lastCursor.y  - 1))
    --steps
